...
static plutosvg_document_t* plutosvg_document_create(float width, float height, plutovg_destroy_func_t destroy_func, void* closure)
{
    ...
    document->image_cache_limit = PLUTOSVG_IMAGE_CACHE_LIMIT;
...
void plutosvg_document_set_image_cache_limit(plutosvg_document_t* document, size_t limit)
{
    if(document == NULL)
        return;
    document->image_cache_limit = limit;
}
...
{
    if(document->image_cache_bytes <= document->image_cache_limit - bytes)
        return true;
    image_cache_prepare_frequencies(document);
    size_t candidate_value = image_cache_value(candidate->image_access_count,
        bytes);
    if(candidate_value == 0)
        return false;

    size_t required = document->image_cache_bytes
        - (document->image_cache_limit - bytes);
    size_t freed = 0;
    ...
    plutosvg_document_t* document)
{
    ...
    if(stride > 0 && height > 0
        ...
        if(image_bytes <= document->image_cache_limit
