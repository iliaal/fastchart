...
/* Measure a UTF-8 string at the given point size + DPI using FreeType
 * directly. out_w / out_h receive the pixel-space width and the
 * ascender-to-descender height. Returns 0 on success, -1 on FT failure.
 * Width sums each glyph's advance.x (in 26.6 fixed-point pixels).
 * Height is the face's bbox in pixel units at the configured size. */
...
{
    int w, h;
    if (fc_ft_measure(font_path, font_size, dpi, "Mg9j", &w, &h) != 0) {
                           ...
                           char *err_buf, size_t err_buf_n)
{
    ...
    if (strchr(text, '\n')) {
        ...
        while (1) {
            ...
            if (len > 0) {
                int w = 0, h = 0;
                if (fc_ft_measure(font_path, font_size, dpi, buf, &w, &h) == 0) {
                    if (w > max_w) max_w = w;
    ...
    int w = 0, h = 0;
    if (fc_ft_measure(font_path, font_size, dpi, text, &w, &h) != 0) {
        if (err_buf && err_buf_n) {
