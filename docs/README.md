...
Each example is a self-contained PHP script that renders the chart
shown with it. Run a script with only `fastchart` loaded; ext/gd is
not required:
...
The 71 example scripts cover every chart and symbol family. The PHPT
suite under `tests/` covers the full public API and edge cases.

Every script `require`s `_bootstrap.php`, which finds a TrueType font
(DejaVu on common Linux distros, Helvetica on macOS) and stores its
path in `$font`. Each chart passes it to `->setFontPath($font)`. If
your fonts live elsewhere, set `FC_FONT=/path/to/Custom.ttf`.

Re-run a script to regenerate its PNG.
...
fit along Y. Two `addVerticalBand` calls highlight the SLA
threshold.
...
To put a series on the right axis, add `'axis' => 'right'` to its
dict. `LineChart` and `AreaChart` honor it; other chart types ignore it.
...
Use the bytes-returning helpers for HTTP responses, base64 data URIs,
or hashing. SVG defaults to glyph-path output (`SVG_TEXT_PATHS`):
FreeType flattens every text element to `<path>` data, so the SVG
renders identically in any rasterizer. For smaller files with
selectable text, call
`setSvgTextMode(FastChart\Chart::SVG_TEXT_NATIVE)`.

v1.0 removed GIF and AVIF output and `draw($canvas)`, because
fastchart now owns the pixel buffer through plutovg. To combine several charts in
one image, stitch SVG fragments from `drawSvgFragment()` or composite
the `renderPng()` bytes in userland.
...
them. Call `setFloating` before `setSeries`; otherwise the parser
...
given percentage into a single `Other` slice, so dozens of < 1%
slivers don't crowd the legend.
...
- `setPlotRect(x0, y0, x1, y1)`: pin the plot area to fixed pixel
  coordinates, for example to compose two charts on one canvas
...
```php
/* There is no shared canvas to draw onto, so compose multi-chart
 * layouts in SVG and rasterize the result once if needed. */
...
tick, and qualitative background bands into one compact strip. Use
it on dashboards in place of radial gauges.
...
index into `setNodes()`. A node can have several sources, and any
number of intermediate layers is allowed. In the 4-layer e-commerce
flow below, Mobile and Tablet both feed the Samsung node; every
other brand has one source.
...
`FastChart\Code128` implements ISO/IEC 15417 encoding. It switches
between subsets A (control + uppercase), B (printable ASCII), and C
(digit pairs) within one payload to minimize encoded length, and
appends the mod-103 checksum. `setShowText(true)` prints the payload
below the bars in the auto-detected default font.
...
recovery), M (~15%, default), Q (~25%), H (~30%). Higher ECC uses
more codeword space and may push the encoder up a version (more
modules per side), but the payload survives more damage. Use H for
codes printed on rough surfaces or overlaid with a logo, and L for
clean digital use.
...
Symbol classes only render; they can't draw onto an existing canvas.
To composite a symbol onto an image, load the encoded bytes with
`imagecreatefromstring()` in userland.

---

## Common patterns

- `renderToFile($path)` infers the format from the extension (.png,
  .jpg, .jpeg, .webp, .svg). For raw bytes (HTTP, hashing, base64,
  in-memory composition), use `renderPng()`, `renderJpeg($q)`,
  `renderWebp($q)`, or `renderSvg()`.
- `setSvgTextMode(SVG_TEXT_PATHS)` (default) emits glyphs as `<path>`
  outlines, so the SVG is self-contained. `SVG_TEXT_NATIVE` emits
  `<text>` elements: smaller files and selectable text, but the
  consumer's renderer must support SVG text.
- `setJpegQuality(int)` sets the per-instance default (1..100,
  default 88) for `renderJpeg()` with no argument and
  `renderToFile('*.jpg')`. `renderJpeg($quality)` overrides it per
  call.
- Some setters must run before `setSeries` so the parser knows the
  input shape. The most common is `setFloating(true)`; without it,
  the parser expects scalars, not `[min, max]` tuples.

### Strict mode and best-effort parsing

`setStrict(true)` applies to `setSeries()` on `LineChart`,
`AreaChart`, and `BarChart`, and to `Funnel::setStages()`. Those
setters then throw `TypeError` on malformed cells. Every other setter
that takes an array of data (slices, points, OHLCV rows, tasks,
boxes, grids, vectors, columns, and so on) is best-effort: it
silently drops bad entries or coerces them to NaN, and never raises
`TypeError`, even with `setStrict(true)`.

Best-effort covers malformed entries, not oversized counts. An array
larger than a setter's documented cap throws `ValueError` regardless
of strict mode, instead of being truncated to the cap and rendering
as if it were complete.

This is deliberate: Gantt task lists and Sankey flows are often
assembled from mixed sources, and dropping one bad row is usually
better than aborting the render. Tests such as
`193_strict_best_effort_pie.phpt` and
`194_strict_best_effort_gantt.phpt` cover this behavior.

If you need hard validation for other chart types, validate the
array yourself before passing it to the setter.
